...
export default class SlackAssistant extends Server implements CustomTransportStrategy {
  ...
  async listen(callback: () => void): Promise<void> {
    ...
    for (const key of this.#eventsBuffer.keys()) {
      this.#app.client.addListener(key as WebClientEvent, this.#eventsBuffer.get(key) as Handler);
    }
    ...
    this.#app.assistant(new Assistant({
      ...
      threadStarted: this.getHandlerByPattern('threadStarted') as AssistantThreadStartedMiddleware ?? noop,
      threadContextChanged: this.getHandlerByPattern('threadContextChanged') as AssistantThreadContextChangedMiddleware ?? noop,
    }));
...
// todo web-client/WebClient events for better typing of the event argument
  on(event: string, callback: Func): void {
    if (this.#app) {
      this.#app.client.addListener(event as WebClientEvent, callback as Handler);
    } else {
